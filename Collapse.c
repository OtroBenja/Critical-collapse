...
double RH_aK(double r, double a, double Phi2, double Pi2){
    //double _a2 = 1/(a*a);
    //double dif2 = Pi2 -Phi2;
    //return a*( 4*PI*r*(Pi2*(1+BETA*dif2*_a2) -0.5*dif2*(1+0.5*BETA*_a2)) -0.5*(a*a-1)/r );
    double X = 0.5*(Pi2-Phi2)/(a*a);
    return a*( 4*PI*r*(Pi2*(1+2*BETA*X) -a*a*X*(1+BETA*X)) +0.5*(a*a-1)/r );
}

double RH_alphaK(double r, double a, double alpha, double Phi2, double Pi2){
    //double _a2 = 1/(a*a);
    //double dif2 = Pi2 - Phi2;
    //return alpha*( 4*PI*r*(Phi2*(1+BETA*dif2*_a2) +0.5*dif2*(1+0.5*BETA*_a2)) +0.5*(a*a-1)/r );
    double X = 0.5*(Pi2-Phi2)/(a*a);
    return alpha*( 4*PI*r*(Phi2*(1+2*BETA*X) +a*a*X*(1+BETA*X)) +0.5*(a*a-1)/r );
}

double RH_PiK(double r, double a, double alpha, double Pi, double Phi, 
              double* Epsilon, double* Gamma, int ir, double deltaR){
    double _a2 = 1/(a*a);
    double X = 0.5*(Pi*Pi-Phi*Phi)*_a2;
    return ( centered_D1(Epsilon, ir, deltaR)/(r*r) + 16*BETA*PI*r*Gamma[ir]*Phi*Pi*X*(1+2*BETA*X)
        +2*BETA*Phi*Pi*centered_D1(Gamma,ir,deltaR)*_a2 )/(1+2*BETA*X+2*BETA*Pi*Pi*_a2);

}
double RH_PiK_leftmid(double r, double a, double alpha, double Pi, double Phi, 
              double* Epsilon, double* Gamma, int ir, double deltaR){
    double _a2 = 1/(a*a);
    double X = 0.5*(Pi*Pi-Phi*Phi)*_a2;
    return ( leftmid_D1(Epsilon, ir, deltaR)/(r*r) + 16*BETA*PI*r*Gamma[ir]*Phi*Pi*X*(1+2*BETA*X)
        +2*BETA*Phi*Pi*leftmid_D1(Gamma,ir,deltaR)*_a2 )/(1+2*BETA*X+2*BETA*Pi*Pi*_a2);

}
double RH_PiK_rightmid(double r, double a, double alpha, double Pi, double Phi, 
              double* Epsilon, double* Gamma, int ir, double deltaR){
    double _a2 = 1/(a*a);
    double X = 0.5*(Pi*Pi-Phi*Phi)*_a2;
    return ( rightmid_D1(Epsilon, ir, deltaR)/(r*r) + 16*BETA*PI*r*Gamma[ir]*Phi*Pi*X*(1+2*BETA*X)
        +2*BETA*Phi*Pi*rightmid_D1(Gamma,ir,deltaR)*_a2 )/(1+2*BETA*X+2*BETA*Pi*Pi*_a2);

}
double RH_PiK_r0(double alpha, double Pi2, double* Phi, double deltaR){
    return alpha*(1+BETA*Pi2)*leftmost_D1(Phi,0,deltaR);
}
...
double** iteration_old(double* r,double* phi,double* Phi,double* Pi,double deltaR,int maxR,int iterations,int save_iteration,double epsilon){
    ...
    for(int i=0;i<iterations;i++){
       ...
       //calculate jn, kn and ln
        for(int n=0;n<4;n++){
            ...
            j_n[0] = Gamma[0];
            k_n[0] = 0;
            l_n[0] = alpha[0]*leftmost_D1(Phi_rk,0,deltaR);
            j_n[1] = Gamma[1];
            k_n[1] = leftmid_D1(Gamma,1,deltaR);
            l_n[1] = leftmid_D1(Epsilon,1,deltaR)/r2[1];
            //#pragma omp parallel for
...
double** iterationK(double* r,double* phi,double* Phi,double* Pi,double deltaR,int maxR,int iterations,int save_iteration,double epsilon){
    ...
    double *a = malloc(sizeof(double)*nR);
    double *alpha = malloc(sizeof(double)*nR);
    double *Phi2 = malloc(sizeof(double)*nR);
    double *Pi2 = malloc(sizeof(double)*nR);
    double *Phi2_p05 = malloc(sizeof(double)*nR);
    double *Pi2_p05 = malloc(sizeof(double)*nR);
    double *Gamma = malloc(sizeof(double)*nR);
    double *Epsilon = malloc(sizeof(double)*nR);
    double *Phi_rk = malloc(sizeof(double)*nR);
    double * Pi_rk = malloc(sizeof(double)*nR);
        ...
        if(SAVE_MODE == 0){
            ...
            if(save_count == save_iteration){
                ...
                if(MASS){
                    mass = 0;
                    for(int ir=0;ir<nR;ir++){
                        mass += 2*PI*r2[ir]*(Phi2[ir]+Pi2[ir])/(a[ir]*a[ir])*deltaR;
                    }
                    Mhistory[i/save_iteration] = mass;
                }
                for(int ir=0;ir<(nR/SAVE_RES);ir++){
       ...
       //calculate jn, kn and ln
        for(int n=0;n<4;n++){
            for(int ir=0;ir<nR;ir++){
                Phi_rk[ir] = Phi[ir]+_rk[n]*deltaT*k_n[ir];
                 Pi_rk[ir] =  Pi[ir]+_rk[n]*deltaT*l_n[ir];
                Phi2[ir] = (Phi_rk[ir])*(Phi_rk[ir]);
                 Pi2[ir] = (Pi_rk[ir])*(Pi_rk[ir]);
            }
            for(int ir=0;ir<nR-1;ir++){
                Phi2_p05[ir] = 0.25*(Phi_rk[ir] +Phi_rk[ir+1])*(Phi_rk[ir] +Phi_rk[ir+1]);
                 Pi2_p05[ir] = 0.25*( Pi_rk[ir] + Pi_rk[ir+1])*( Pi_rk[ir] + Pi_rk[ir+1]);
            }
            //First iterate the metric for this Runge-Kutta step
            for(int ir=0;ir<nR-1;ir++){
                //calculate m1 and n1
                m1 = deltaR*    RH_aK(r[ir],a[ir],          Phi2[ir],Pi2[ir]);
                n1 = deltaR*RH_alphaK(r[ir],a[ir],alpha[ir],Phi2[ir],Pi2[ir]);
                //calculate m2 and n2
                m2 = deltaR*    RH_aK(r[ir]+0.5*deltaR,a[ir]+0.5*m1,                 Phi2_p05[ir],Pi2_p05[ir]);
                n2 = deltaR*RH_alphaK(r[ir]+0.5*deltaR,a[ir]+0.5*m1,alpha[ir]+0.5*n1,Phi2_p05[ir],Pi2_p05[ir]);
                //calculate m3 and n3
                m3 = deltaR*    RH_aK(r[ir]+0.5*deltaR,a[ir]+0.5*m2,                 Phi2_p05[ir],Pi2_p05[ir]);
                n3 = deltaR*RH_alphaK(r[ir]+0.5*deltaR,a[ir]+0.5*m2,alpha[ir]+0.5*n2,Phi2_p05[ir],Pi2_p05[ir]);
                //calculate m4 and n4
                m4 = deltaR*    RH_aK(r[ir+1],a[ir]+m3,             Phi2[ir+1],Pi2[ir+1]);
                n4 = deltaR*RH_alphaK(r[ir+1],a[ir]+m3,alpha[ir]+n3,Phi2[ir+1],Pi2[ir+1]);
                //Calculate next step for a and alpha
                a[ir+1]     = a[ir]     +(m1 +2.0*(m2+m3) +m4)*_6;
                alpha[ir+1] = alpha[ir] +(n1 +2.0*(n2+n3) +n4)*_6;
            }
            for(int ir=0;ir<nR;ir++){
                Gamma[ir]   =        alpha[ir]*( Pi_rk[ir])/a[ir];
                Epsilon[ir] = r2[ir]*alpha[ir]*(Phi_rk[ir])*(1+2*BETA*(Phi2[ir]-Pi2[ir]))/a[ir];
            }
            for(int ir=0;ir<5;ir++){
                rPhi[ir] = r[nR-5 +ir]*Phi_rk[nR-5 +ir];
                 rPi[ir] = r[nR-5 +ir]* Pi_rk[nR-5 +ir];
            ...
            l_n[0] = RH_PiK_r0(alpha[0],Pi2[0],Phi_rk,deltaR);
            j_n[1] = Gamma[1];
            k_n[1] = leftmid_D1(Gamma,1,deltaR);
            //l_n[1] = leftmid_D1(Epsilon,1,deltaR)/r2[1];
            l_n[1] = RH_PiK_leftmid(r[1],a[1],alpha[1],Pi_rk[1],Phi_rk[1],Epsilon,Gamma,1,deltaR);
            //#pragma omp parallel for
            for(int ir=2;ir<nR-2;ir++){
                j_n[ir] = Gamma[ir];
                k_n[ir] = centered_D1(Gamma,ir,deltaR);
                //l_n[ir] = centered_D1(Epsilon,ir,deltaR)/r2[ir];
                l_n[ir] = RH_PiK_leftmid(r[ir],a[ir],alpha[ir],Pi_rk[ir],Phi_rk[ir],Epsilon,Gamma,ir,deltaR);
            }
            j_n[nR-2] = Gamma[nR-2];
            k_n[nR-2] = rightmid_D1(Gamma,nR-2,deltaR);
            //l_n[nR-2] = rightmid_D1(Epsilon,nR-2,deltaR)/r2[nR-2];
            l_n[nR-2] = RH_PiK_leftmid(r[nR-2],a[nR-2],alpha[nR-2],Pi_rk[nR-2],Phi_rk[nR-2],Epsilon,Gamma,nR-2,deltaR);
            j_n[nR-1] = Gamma[nR-1];
            k_n[nR-1] = -rightmost_D1(rPhi,4,deltaR)/r[nR-1];
            l_n[nR-1] = -rightmost_D1( rPi,4,deltaR)/r[nR-1];
