...
void metric_iteration_NW(double *a0_p,double *alpha0_p,double* Beta, double* Beta1_2, double* a, double* alpha, double* r, int nR, double deltaR, double normalize){
    
    double m1, n1, m2, n2, m3, n3, m4, n4, norm_value;
    double     a0 = *a0_p;
    double alpha0 = *alpha0_p;
    for(int ir=0;ir<nR-1;ir++){
        //calculate m1 and n1
        m1 = deltaR*    a[ir]*(Beta[ir]-0.5*(a[ir]*a[ir]-1)/r[ir]);
        n1 = deltaR*alpha[ir]*(Beta[ir]+0.5*(a[ir]*a[ir]-1)/r[ir]);
        ...
        n4 = deltaR*(alpha[ir]+n3)*(Beta[ir+1]+0.5*((a[ir]+m3)*(a[ir]+m3)-1)/r[ir+1]);
        //Calculate next step for a and alpha
            a0 +=     a[ir] +(m1 +2.0*(m2+m3) +m4)/6.0;
        alpha0 += alpha[ir] +(n1 +2.0*(n2+n3) +n4)/6.0;
    }
    if (normalize == 0.0) {norm_value = 1.0/(alpha0*a0);
    } else {norm_value = normalize;}
    for(int ir=0;ir<nR;ir++){
        alpha[ir] = alpha[ir]*norm_value;
    }
